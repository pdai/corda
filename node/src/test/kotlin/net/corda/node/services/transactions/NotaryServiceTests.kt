...
class NotaryServiceTests {
    ...
    fun `should report conflict when inputs are reused across transactions`() {
        ...
        with(notaryError.doubleSpendConflict) {
            assertEquals(conflictCauses.size, 2)
            assertEquals(conflictCauses[firstState.ref]!!.transactionIdHash, firstSpendTx.id.sha256())
            assertEquals(conflictCauses[secondState.ref]!!.transactionIdHash, secondSpendTx.id.sha256())
        }
    }
